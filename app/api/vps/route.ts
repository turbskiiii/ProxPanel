...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validation.success) {
      return NextResponse.json({ error: "Invalid configuration", details: validation.error.issues }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: error.message || "VPS creation failed" }, { status: 500 })
  }
}
