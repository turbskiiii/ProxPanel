...
export default function CreateVPSPage() {
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 2:
        return formData.cpu >= 1 && formData.memory >= 512 && formData.disk >= 10
      case 3:
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => router.push("/dashboard/vps")}>
        ...
        <Card className="max-w-2xl mx-auto">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">VPS Created Successfully!</CardTitle>
            <CardDescription>Your virtual private server is being set up</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              ...
              {createdVPS?.rootPassword && (
                <div className="flex justify-between">
                  ...
                  <code className="bg-gray-200 px-2 py-1 rounded text-sm">{createdVPS.rootPassword}</code>
                </div>
              )}
            </div>
            ...
            <Alert>
              ...
              <AlertDescription>
                Your VPS is being created in the background. This process may take 2-5 minutes. You can monitor the
                progress from the VPS management page.
              </AlertDescription>
            </Alert>

            <div className="flex gap-3">
              <Button onClick={() => router.push(`/dashboard/vps/${createdVPS?.id}`)} className="flex-1">
                <Server className="mr-2 h-4 w-4" />
                Manage VPS
              </Button>
              <Button variant="outline" onClick={() => router.push("/dashboard/vps")} className="flex-1">
                View All VPS
              </Button>
            </div>
          </CardContent>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Set up your virtual private server in minutes</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose a name and operating system for your VPS</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">Use letters, numbers, hyphens, and underscores only</p>
              </div>
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {template.recommended && <Badge variant="secondary">Recommended</Badge>}
                          </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Allocate CPU, memory, and storage for your VPS</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* CPU Configuration */}
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, cpu: Math.max(1, formData.cpu - 1) })}
                    disabled={formData.cpu <= 1}
                  >
                    -
                  </Button>
                  <span className="w-16 text-center font-mono">{formData.cpu} cores</span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, cpu: Math.min(16, formData.cpu + 1) })}
                ...
                <p className="text-xs text-muted-foreground">
                  Minimum: {selectedTemplate?.minCpu || 1} core(s) • Maximum: 16 cores
                </p>
              </div>
              ...
              <div className="space-y-3">
                <Label className="flex items-center gap-2">
                  ...
                  Memory: {formData.memory}MB ({(formData.memory / 1024).toFixed(1)}GB)
                </Label>
                <div className="flex items-center gap-4">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, memory: Math.max(512, formData.memory - 512) })}
                    disabled={formData.memory <= 512}
                  >
                    -512MB
                  </Button>
                  <span className="w-24 text-center font-mono">{formData.memory}MB</span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, memory: Math.min(32768, formData.memory + 512) })}
                ...
                <p className="text-xs text-muted-foreground">
                  Minimum: {selectedTemplate?.minMemory || 512}MB • Maximum: 32GB
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, disk: Math.max(10, formData.disk - 10) })}
                    disabled={formData.disk <= 10}
                  >
                    -10GB
                  </Button>
                  <span className="w-16 text-center font-mono">{formData.disk}GB</span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setFormData({ ...formData, disk: Math.min(1000, formData.disk + 10) })}
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-blue-900">Estimated Monthly Cost</span>
                </div>
                <div className="text-2xl font-bold text-blue-900">${calculateCost().toFixed(2)}/month</div>
                <div className="text-sm text-blue-700 mt-1">
                  CPU: ${(formData.cpu * 5).toFixed(2)} • RAM: ${((formData.memory / 1024) * 8).toFixed(2)} • Storage: $
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose the datacenter location for your VPS</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {mockNodes.map((node) => (
                  <div
                    ...
                    onClick={() => node.status === "online" && setFormData({ ...formData, node: node.id })}
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge variant={node.status === "online" ? "default" : "destructive"}>{node.status}</Badge>
                    </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-lg">${calculateCost().toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  Your VPS will be created with a randomly generated root password. You can change this password after
                  creation from the VPS management panel.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
