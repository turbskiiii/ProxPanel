...
export class BackupService {
  ...
  async createBackup(
    vpsId: string,
    type: 'snapshot' | 'backup' = 'snapshot'
  ): Promise<string> {
    try {
      // Get VPS details
      const vpsResult = await query(
        'SELECT vmid, node_id, name FROM vps_instances WHERE id = ?',
        [vpsId]
      );
  ...
  async restoreBackup(vpsId: string, backupId: string): Promise<string> {
    try {
      ...
      const backupResult = await query(
        'SELECT * FROM backups WHERE id = ? AND vps_id = ?',
        [backupId, vpsId]
      );
      ...
      const vpsResult = await query(
        'SELECT vmid, node_id FROM vps_instances WHERE id = ?',
        [vpsId]
      );
      const vps = vpsResult[0];
      ...
      if (backup.type === 'snapshot') {
        ...
        taskId = await this.restoreFromSnapshot(
          vps.node_id,
          vps.vmid,
  ...
  private async cleanupOldBackups(): Promise<void> {
    try {
      ...
      const configs = await query(
        'SELECT vps_id, retention FROM backup_configs WHERE enabled = 1'
      );
