...
export class EmailService {
  ...
  async sendVPSCreated(userEmail: string, vpsDetails: any): Promise<boolean> {
    try {
      ...
      logger.info("VPS creation email sent", { userEmail, vpsId: vpsDetails.id })
      return true
    } catch (error) {
      logger.error("Failed to send VPS creation email", { error: error.message, userEmail })
      return false
    }
  }
  ...
  async sendVPSStatusChange(
    ...
    try {
      const template = this.getStatusChangeTemplate(vpsDetails, oldStatus, newStatus)
      ...
      logger.info("VPS status change email sent", { userEmail, vpsId: vpsDetails.id, oldStatus, newStatus })
      return true
    } catch (error) {
      logger.error("Failed to send status change email", { error: error.message, userEmail })
      return false
    }
  }

  async sendMaintenanceNotification(userEmail: string, maintenanceDetails: any): Promise<boolean> {
    try {
      ...
      logger.error("Failed to send maintenance notification", { error: error.message, userEmail })
      return false
    }
  }
...
Manage your VPS: ${process.env.APP_URL}/dashboard/vps/${vpsDetails.id}`,
  ...
  private getStatusChangeTemplate(vpsDetails: any, oldStatus: string, newStatus: string): EmailTemplate {
    const statusEmoji = newStatus === "running" ? "🟢" : newStatus === "stopped" ? "🔴" : "🟡"
