...
export class ProxmoxClient {
        ...
        res.on("end", () => {
          try {
            ...
            if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
              ...
              reject(new Error(`HTTP ${res.statusCode}: ${parsed.errors || responseData}`))
            }
          } catch (error) {
  ...
  async getVMConfig(node: string, vmid: number): Promise<any> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/config`)
    return response.data
  }

  // Get VM status
  async getVMStatus(node: string, vmid: number): Promise<any> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/status/current`)
    return response.data
  }
  ...
  async startVM(node: string, vmid: number): Promise<string> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/status/start`, "POST")
    return response.data
  }

  // Stop VM
  async stopVM(node: string, vmid: number): Promise<string> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/status/stop`, "POST")
    return response.data
  }

  // Reboot VM
  async rebootVM(node: string, vmid: number): Promise<string> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/status/reboot`, "POST")
    return response.data
  }

  // Delete VM
  async deleteVM(node: string, vmid: number): Promise<string> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}`, "DELETE")
    return response.data
  }
  ...
  async getTemplates(node: string, storage: string): Promise<any[]> {
    const response = await this.makeRequest(`/nodes/${node}/storage/${storage}/content?content=vztmpl`)
    return response.data || []
  }

  // Clone VM from template
  async cloneVM(node: string, vmid: number, newid: number, config: any): Promise<string> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/clone`, "POST", {
      newid,
      ...config,
    })
    return response.data
  ...
  async getTaskStatus(node: string, upid: string): Promise<any> {
    const response = await this.makeRequest(`/nodes/${node}/tasks/${upid}/status`)
    return response.data
  }

  // Wait for task completion
  async waitForTask(node: string, upid: string, timeout = 300000): Promise<boolean> {
  ...
  async getVMMetrics(node: string, vmid: number): Promise<any> {
    const response = await this.makeRequest(`/nodes/${node}/qemu/${vmid}/rrddata?timeframe=hour`)
    return response.data
  }

  // Set VM password
  async setVMPassword(node: string, vmid: number, username: string, password: string): Promise<boolean> {
    try {
      await this.makeRequest(`/nodes/${node}/qemu/${vmid}/agent/set-user-password`, "POST", {
        username,
        password,
      })
      return true
