...
export class VPSService {
  ...
  async createVPS(userId: number, config: VPSConfig): Promise<any> {
    try {
      ...
      logger.error("VPS creation failed", { error: error.message, userId, config })
    ...
    try {
      ...
      const vpsWithStatus = await Promise.all(
        vpsInstances.map(async (vps: any) => {
          try {
            const proxmoxStatus = await this.proxmox.getVMStatus(vps.node_id, vps.vmid)
            const proxmoxConfig = await this.proxmox.getVMConfig(vps.node_id, vps.vmid)

            // Update database if status changed
            if (proxmoxStatus.status !== vps.status) {
              await query("UPDATE vps_instances SET status = ?, updated_at = NOW() WHERE id = ?", [
            ...
            return {
              ...
              memory_usage: proxmoxStatus.mem ? (proxmoxStatus.mem / proxmoxStatus.maxmem) * 100 : 0,
              network: {
            ...
            logger.warn(`Failed to get status for VPS ${vps.id}`, { error: error.message })
            return vps
          }
        }),
      )
      ...
      logger.error("Failed to get user VPS list", { error: error.message, userId })
      throw error
    }
  }
  ...
  async getVPSDetails(userId: number, vpsId: string): Promise<any> {
    try {
      ...
      logger.error("Failed to get VPS details", { error: error.message, vpsId, userId })
      throw error
    }
  }

  async controlVPS(userId: number, vpsId: string, action: "start" | "stop" | "reboot"): Promise<boolean> {
    try {
      ...
      if (success) {
        ...
        const newStatus = action === "start" || action === "reboot" ? "running" : "stopped"
      ...
      logger.error(`VPS ${action} failed`, { error: error.message, vpsId, userId })
  ...
  async deleteVPS(userId: number, vpsId: string): Promise<boolean> {
    try {
      ...
      try {
        ...
        logger.warn("Failed to stop VPS before deletion", { error: error.message })
      }
      ...
      logger.error("VPS deletion failed", { error: error.message, vpsId, userId })
        ...
        onlineNodes.map(async (node) => {
          try {
            ...
            return {
              node: node.node,
              cpu: status.cpu || 0,
              memory: status.memory ? status.memory.used / status.memory.total : 0,
            }
          } catch (error) {
  ...
  async resetVPSPassword(userId: number, vpsId: string): Promise<string> {
    try {
      ...
      const success = await this.proxmox.setVMPassword(vps.node_id, vps.vmid, "root", newPassword)
      ...
      logger.error("VPS password reset failed", { error: error.message, vpsId, userId })
      throw error
    }
  }
}
